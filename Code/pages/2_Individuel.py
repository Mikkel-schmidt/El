...
if not st.session_state.valgt_meter.any():
...
daytime = df.groupby([df['from'].dt.date, 'day-moment']).mean()
daytime = daytime.reset_index()



def moving_avg(df, win):
    moving_average = df['amount'].rolling(
        window=win,       
        center=True,      # puts the average at the center of the window
        min_periods=int(np.floor(win/2)),  # choose about half the window size
    ).mean() 
...
dff = df.groupby('from').agg({'meter': 'mean', 'amount': 'sum'}).reset_index()
mybkps = rupt('day', dff)

value_avg_day = np.zeros(len(dff['amount']))
stdd_avg_day  = np.zeros(len(dff['amount']))
vvvv = np.zeros((len(mybkps),2),)
k=0
j=0
for i in mybkps:
...
def linesss(df, dff):
    b1 = (
        ...
        linestyle_opts=opts.LineStyleOpts( width=1), #color="#033832",
        #itemstyle_opts=opts.ItemStyleOpts(border_width=0.1, border_color="yellow", color="blue")
        )
        #.add_yaxis('Dagligt gennemsnit', mavg, label_opts=opts.LabelOpts(is_show=False,formatter="{b}: {c}"),
        #linestyle_opts=opts.LineStyleOpts( width=3))
        .add_yaxis('Activity', list(dff['bkps']),  label_opts=opts.LabelOpts(is_show=False,formatter="{b}: {c}"),
        linestyle_opts=opts.LineStyleOpts( width=5),symbol='emptyCircle', symbol_size=10)
        #.add_yaxis('Baseload', list(xinterp),  label_opts=opts.LabelOpts(is_show=False,formatter="{b}: {c}"),
        #linestyle_opts=opts.LineStyleOpts( width=5),)
        # markpoint_opts=opts.MarkPointOpts(
        #     data=[opts.MarkPointItem(type_="max", name="Højeste drift")])
        

        #.add_yaxis('Forbrug sidste år', list(y2), symbol='emptyCircle', symbol_size=2, label_opts=opts.LabelOpts(is_show=False,formatter="{b}: {c}"),
...
with col2:
    ...
    figur = linesss(dff, dff)
    st_pyecharts(figur, height='600px')

    # fig, ax = plt.subplots(figsize=(14,6))
    # ax.plot(dff['from'],dff['amount'], linewidth=0.3)
    # #ax.step(vvvv['datetime'], vvvv.iloc[:,0], where='post')
...
if df['meter'].nunique() > 1:
    with st.expander("Se individuelle målere:"):
        bimaaler = st.selectbox('Vælg bimåler:', df['meter'].unique())
        figur = linesss(df[df['meter']==bimaaler], dff)
        st_pyecharts(figur, height='600px')
...
#st.write(df_norm)
# st.write(vvvv)
# st.write(int(vvvv[np.argmin(vvvv[:,0]),1]))
# st.write(int(vvvv[np.argmin(vvvv[:,0])+1,1]))
# st.write(int(vvvv[-1,1]))
# st.write(np.argmin(vvvv[:,0]))
# st.write(vvvv.shape[0])
# for i in range(4):
#     st.write(df.iloc[int(vvvv[i,1]),1])
